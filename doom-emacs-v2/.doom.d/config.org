...
#+PROPERTY: header-args :tangle config.el :results output silent
#+auto_tangle: t
#+STARTUP: overview
...
** Temp. packages
This is where I store configs for packages that I don't know which heading to put under yet, the package will be refiled under the appropriate heading some other time.
** Code linting
*** Flycheck
#+begin_src emacs-lisp
(after! flycheck
...
** Completion
*** Cape
CAPE (Completion At Point Extensions) provides a way to complete things like file names and other stuff.
#+begin_src emacs-lisp
(use-package! cape
  :config
...
*** Vertico multiform
Vertico has extensions to dynamically change what the completion area looks like, I define keybinds for it here and also what commands will use which style.
#+begin_src emacs-lisp
...
*** Lin
Lin provides a customizable alternative to hl-line.
#+begin_src emacs-lisp
(use-package! lin
  :init
...
** Utilities
*** Org-auto-tangle
Automatically tangle an Org file on save, rather than having to do it manually.
#+begin_src emacs-lisp
